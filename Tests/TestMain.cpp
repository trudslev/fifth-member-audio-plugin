...
int main()
{
    // ProgramManager defers every apply through juce::AsyncUpdater, and triggerAsyncUpdate() needs
    // a MessageManager: without one, activeMessage->post() fails, the pending flag is cleared, and
    // handleUpdateNowIfNeeded() has nothing to deliver. The failure is silent - no assertion in a
    // release build, just a program change that never arrives - so any test of that path would pass
    // while proving nothing. This initialiser is what makes those tests real.
