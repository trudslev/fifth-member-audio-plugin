...
namespace
{
    ...
    */
    ...
    {
    public:
        LayoutHost() : apvts (*this, nullptr, ParamIDs::stateTreeType, createFifthMemberParameterLayout()) {}
...
public:
    ...
    void runTest() override
    {
        beginTest ("Every parameter prints cleanly across its range");
        {
            ...
            for (const float position : { 0.0f, 0.13f, 0.37f, 0.5f, 0.61f, 0.89f, 1.0f })
            {
                for (auto* raw : host.getParameters())
                {
                    ...
                    const auto defects = nf::readoutDefects (*p, FifthMemberTheme::Layout::readoutFormat());
        ...
        beginTest ("The readout fits the LCD's character budget at every position");
        {
            ...
            LayoutHost host;
            const int budget = FifthMemberTheme::Layout::lcdCharacterBudget + 5;
            ...
            for (const float position : { 0.0f, 0.37f, 1.0f })
                for (auto* raw : host.getParameters())
                    if (auto* p = dynamic_cast<juce::AudioProcessorParameterWithID*> (raw))
                    {
                        ...
                        const auto text = nf::describeParameter (*p, FifthMemberTheme::Layout::readoutFormat());
