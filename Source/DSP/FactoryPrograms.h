...
/** **Program identity comes from core, and these aliases are the whole of the local surface.**
    ...
    Program and the filename for a User one, and `displayName` is carried for presentation and
    deliberately left out of `operator==` so a corrected typo in the bank cannot make a Program
    stop equalling itself.

    Aliased rather than used qualified at every call site, because the unqualified names are what
    the panel, the header and the tests already read, and renaming several hundred uses would bury
    the one change that matters in noise. */
